...
import { Mail, ArrowDown, Code, Zap } from '@lucide/vue'
...
const profile = computed<HeroProfile>(() => ({
  ...
  socialLinks: [
    ...
    {
      platform: 'linkedin',
      url: 'https://linkedin.com/in/geraldine-jc-chen/',
      labelKey: 'hero.social.linkedin',
    },
    {
      platform: 'email',
      url: 'mailto:geraldine.jc.chen@gmail.com',
  ...
  techBadges: [
    ...
    // { name: 'Vuetify', category: 'UI Component' },
...
<template>
  ...
  >
    ...
    <div class="absolute inset-0 pointer-events-none overflow-hidden">
      <div
        class="absolute -top-40 -left-40 w-96 h-96 bg-primary/20 rounded-full blur-3xl opacity-70 animate-pulse"
      ></div>
      <div
        class="absolute top-1/2 -right-40 w-96 h-96 bg-secondary/20 rounded-full blur-3xl opacity-70 animate-pulse"
        style="animation-delay: 1.5s"
      ></div>
      <div
        class="absolute bottom-10 left-1/3 w-80 h-80 bg-accent/15 rounded-full blur-3xl opacity-60"
      ></div>
    </div> -->
    ...
    <div class="my-auto max-w-6xl w-full z-10 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
      ...
      <div class="lg:col-span-7 flex flex-col items-start space-y-6 text-left">
        ...
        <!-- <div
          class="inline-flex items-center gap-2.5 px-4 py-2 rounded-full bg-base-200/90 border border-base-content/10 shadow-sm backdrop-blur-sm hover:border-primary/40 transition-colors"
        >
        ...
        <div class="space-y-3">
          <!-- <div
            class="inline-flex items-center gap-2 text-sm font-semibold tracking-wider text-primary uppercase"
          >
          ...
          <h1 class="text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight leading-none">
            <span
              class="bg-gradient-to-r from-base-content via-primary to-secondary bg-clip-text text-transparent whitespace-pre-wrap"
            >
              {{ t(profile.sloganKey) }}
            </span>
          </h1>
          ...
          <p class="text-lg sm:text-xl font-medium text-primary/90 pt-1">
            {{ t(profile.nameKey) }} | {{ t(profile.titleKey) }}
          </p>
        </div>
        ...
        <div class="w-full pt-2">
          <p
            class="text-xs font-semibold uppercase tracking-wider text-base-content/50 mb-3 flex items-center gap-1.5"
          >
        ...
        <div
          class="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 pt-4 w-full sm:w-auto"
        >
          <!-- Primary CTA: Resume -->
          <!-- <button
    ...
    <div
      class="mt-8 z-10 flex flex-col items-center gap-2 text-base-content/50 hover:text-primary transition-colors cursor-pointer group"
      @click="scrollToContact"
    >
      <span class="text-xs font-medium tracking-wider uppercase font-mono">{{
        t('hero.scrollDown')
      }}</span>
